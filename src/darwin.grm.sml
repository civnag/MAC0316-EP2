...
fun expr_PROD_5_ACT (funcs_string, funcs_string_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps, v, ts) = 
  ( Grammar.Primitivo (Grammar.String_ funcs_string))
fun expr_PROD_6_ACT (val_list, val_list_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps, v, ts) = 
  ( val_list)
...
fun prints_PROD_1_ACT (LP, RP, STR, KW_Print, LP_SPAN : (Lex.pos * Lex.pos), RP_SPAN : (Lex.pos * Lex.pos), STR_SPAN : (Lex.pos * Lex.pos), KW_Print_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps, v, ts) = 
  (  ps := (STR::(!ps)))
fun prints_PROD_2_ACT (ID, LP, RP, KW_Print, ID_SPAN : (Lex.pos * Lex.pos), LP_SPAN : (Lex.pos * Lex.pos), RP_SPAN : (Lex.pos * Lex.pos), KW_Print_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps, v, ts) = 
  ( 
        let 
...
fun prints_PROD_3_ACT (LP, RP, KW_Print, funcs_string, LP_SPAN : (Lex.pos * Lex.pos), RP_SPAN : (Lex.pos * Lex.pos), KW_Print_SPAN : (Lex.pos * Lex.pos), funcs_string_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps, v, ts) = 
  ( ps := (funcs_string)::(!ps))
...
fun funcs_list_PROD_1_ACT (LP, RP, exp_arit1, exp_arit2, val_list, COMMA1, COMMA2, KW_GET, LP_SPAN : (Lex.pos * Lex.pos), RP_SPAN : (Lex.pos * Lex.pos), exp_arit1_SPAN : (Lex.pos * Lex.pos), exp_arit2_SPAN : (Lex.pos * Lex.pos), val_list_SPAN : (Lex.pos * Lex.pos), COMMA1_SPAN : (Lex.pos * Lex.pos), COMMA2_SPAN : (Lex.pos * Lex.pos), KW_GET_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps, v, ts) = 
  ( Grammar.Sample (List.take((List.drop(val_list,getInt(exp_arit2))),getInt(exp_arit1))))
...
fun float_list_PROD_2_ACT (SFLOAT, SFLOAT_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps, v, ts) = 
  ( SFLOAT)
fun exp_bool_PROD_1_ACT (exp_arit1, exp_arit2, rel_op, exp_arit1_SPAN : (Lex.pos * Lex.pos), exp_arit2_SPAN : (Lex.pos * Lex.pos), rel_op_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps, v, ts) = 
  ( Grammar.oper(rel_op,exp_arit1,exp_arit2))
fun exp_bool_PROD_2_ACT (AND, atom_bool1, atom_bool2, AND_SPAN : (Lex.pos * Lex.pos), atom_bool1_SPAN : (Lex.pos * Lex.pos), atom_bool2_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps, v, ts) = 
...
fun funcs_list_PROD_1_PRED (LP, RP, exp_arit1, exp_arit2, val_list, COMMA1, COMMA2, KW_GET, ps, v, ts) = 
  ( (isType exp_arit1 "int") andalso (isType exp_arit2 "int"))
fun exp_bool_PROD_1_PRED (exp_arit1, exp_arit2, rel_op, ps, v, ts) = 
  ( exprTypes exp_arit1 exp_arit2)
fun exp_bool_PROD_2_PRED (AND, atom_bool1, atom_bool2, ps, v, ts) = 
  ( exprTypes atom_bool1 atom_bool2)
fun exp_bool_PROD_3_PRED (OR, atom_bool1, atom_bool2, ps, v, ts) = 
  ( exprTypes atom_bool1 atom_bool2)
...
fun ARGS_58 (exp_bool, KW_IF, ps, v, ts) = 
  (getBool exp_bool)
fun ARGS_61 (b, commands, KW_THEN, ps, v, ts) = 
  (b)
...
fun exp_bool_NT (strm) = let
      fun exp_bool_PROD_1 (strm) = let
            ...
            in
              if (UserCode.exp_bool_PROD_1_PRED (exp_arit1_RES, exp_arit2_RES, rel_op_RES, ps_REFC, v_REFC, ts_REFC))
                then let
                  val FULL_SPAN = (#1(exp_arit1_SPAN), #2(exp_arit2_SPAN))
                  in
                    (UserCode.exp_bool_PROD_1_ACT (exp_arit1_RES, exp_arit2_RES, rel_op_RES, exp_arit1_SPAN : (Lex.pos * Lex.pos), exp_arit2_SPAN : (Lex.pos * Lex.pos), rel_op_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps_REFC, v_REFC, ts_REFC),
                      FULL_SPAN, strm')
                  end
                else fail()
            end
      fun exp_bool_PROD_2 (strm) = let
            ...
            in
              if (UserCode.exp_bool_PROD_2_PRED (AND_RES, atom_bool1_RES, atom_bool2_RES, ps_REFC, v_REFC, ts_REFC))
                then let
                  val FULL_SPAN = (#1(atom_bool1_SPAN), #2(atom_bool2_SPAN))
                  in
                    (UserCode.exp_bool_PROD_2_ACT (AND_RES, atom_bool1_RES, atom_bool2_RES, AND_SPAN : (Lex.pos * Lex.pos), atom_bool1_SPAN : (Lex.pos * Lex.pos), atom_bool2_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps_REFC, v_REFC, ts_REFC),
                      FULL_SPAN, strm')
                  end
                else fail()
            end
      fun exp_bool_PROD_3 (strm) = let
            ...
            val (atom_bool2_RES, atom_bool2_SPAN, strm') = atom_bool_NT(strm')
            in
              if (UserCode.exp_bool_PROD_3_PRED (OR_RES, atom_bool1_RES, atom_bool2_RES, ps_REFC, v_REFC, ts_REFC))
                then let
                  val FULL_SPAN = (#1(atom_bool1_SPAN), #2(atom_bool2_SPAN))
                  in
                    (UserCode.exp_bool_PROD_3_ACT (OR_RES, atom_bool1_RES, atom_bool2_RES, OR_SPAN : (Lex.pos * Lex.pos), atom_bool1_SPAN : (Lex.pos * Lex.pos), atom_bool2_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps_REFC, v_REFC, ts_REFC),
                      FULL_SPAN, strm')
                  end
                else fail()
            end
...
fun val_list_NT (strm) = let
      ...
      in
        (case (lex(strm))
         of (Tok.SSTRING(_), _, strm') => val_list_PROD_4(strm)
...
fun funcs_float_NT (strm) = let
      ...
      in
        (case (lex(strm))
         of (Tok.KW_COV, _, strm') => funcs_float_PROD_9(strm)
...
fun expr_NT (strm) = let
      ...
      fun expr_PROD_5 (strm) = let
            ...
            in
              (UserCode.expr_PROD_5_ACT (funcs_string_RES, funcs_string_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps_REFC, v_REFC, ts_REFC),
                FULL_SPAN, strm')
            end
      fun expr_PROD_6 (strm) = let
            ...
            in
              (UserCode.expr_PROD_6_ACT (val_list_RES, val_list_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps_REFC, v_REFC, ts_REFC),
                FULL_SPAN, strm')
            end
      in
        (case (lex(strm))
         of (Tok.SSTRING(_), _, strm') => expr_PROD_6(strm)
          | (Tok.SINT(_), _, strm') => expr_PROD_6(strm)
          | (Tok.SFLOAT(_), _, strm') => expr_PROD_6(strm)
          | (Tok.SBOOL(_), _, strm') => expr_PROD_6(strm)
          ...
          | (Tok.KW_TOSTRING, _, strm') => expr_PROD_5(strm)
...
and funcs_string_NT (strm) = let
      ...
      in
        (case (lex(strm))
         of (Tok.KW_LINREG, _, strm') => funcs_string_PROD_2(strm)
...
fun prints_NT (strm) = let
      fun prints_PROD_1 (strm) = let
            ...
            in
              (UserCode.prints_PROD_1_ACT (LP_RES, RP_RES, STR_RES, KW_Print_RES, LP_SPAN : (Lex.pos * Lex.pos), RP_SPAN : (Lex.pos * Lex.pos), STR_SPAN : (Lex.pos * Lex.pos), KW_Print_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps_REFC, v_REFC, ts_REFC),
                FULL_SPAN, strm')
            end
      fun prints_PROD_2 (strm) = let
            ...
            val FULL_SPAN = (#1(KW_Print_SPAN), #2(RP_SPAN))
            in
              (UserCode.prints_PROD_2_ACT (ID_RES, LP_RES, RP_RES, KW_Print_RES, ID_SPAN : (Lex.pos * Lex.pos), LP_SPAN : (Lex.pos * Lex.pos), RP_SPAN : (Lex.pos * Lex.pos), KW_Print_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps_REFC, v_REFC, ts_REFC),
                FULL_SPAN, strm')
            end
      fun prints_PROD_3 (strm) = let
            val (KW_Print_RES, KW_Print_SPAN, strm') = matchKW_Print(strm)
            val (LP_RES, LP_SPAN, strm') = matchLP(strm')
            val (funcs_string_RES, funcs_string_SPAN, strm') = funcs_string_NT(strm')
            val (RP_RES, RP_SPAN, strm') = matchRP(strm')
            val FULL_SPAN = (#1(KW_Print_SPAN), #2(RP_SPAN))
            in
              (UserCode.prints_PROD_3_ACT (LP_RES, RP_RES, KW_Print_RES, funcs_string_RES, LP_SPAN : (Lex.pos * Lex.pos), RP_SPAN : (Lex.pos * Lex.pos), KW_Print_SPAN : (Lex.pos * Lex.pos), funcs_string_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ps_REFC, v_REFC, ts_REFC),
                FULL_SPAN, strm')
            end
      in
        (case (lex(strm))
         of (Tok.KW_Print, _, strm') =>
              (case (lex(strm'))
               of (Tok.LP, _, strm') =>
                    (case (lex(strm'))
                     of (Tok.KW_TOSTRING, _, strm') => prints_PROD_3(strm)
                      | (Tok.KW_LINREG, _, strm') => prints_PROD_3(strm)
                      | (Tok.STR(_), _, strm') => prints_PROD_1(strm)
                      | (Tok.ID(_), _, strm') => prints_PROD_2(strm)
                      | _ => fail()
                    (* end case *))
                | _ => fail()
              (* end case *))
...
and conditional_NT (strm) = let
      ...
      val (exp_bool_RES, exp_bool_SPAN, strm') = exp_bool_NT(strm')
      val (block_RES, block_SPAN, strm') = (block_NT (UserCode.ARGS_58 (exp_bool_RES, KW_IF_RES, ps_REFC, v_REFC, ts_REFC)))(strm')
...
and block_NT (b_RES) (strm) = let
      ...
      fun block_PROD_2 (strm) = let
            val (KW_THEN_RES, KW_THEN_SPAN, strm') = matchKW_THEN(strm)
            val (commands_RES, commands_SPAN, strm') = commands_NT(strm')
            val (else_block_RES, else_block_SPAN, strm') = (else_block_NT (UserCode.ARGS_61 (b_RES, commands_RES, KW_THEN_RES, ps_REFC, v_REFC, ts_REFC)))(strm')
