structure DarwinTokens =
  struct
    datatype token
      ...
      | EOF
    val allToks = [
            KW_let, KW_in, KW_title, EQ, PLUS, DOT, EEQ, TIMES, DIV, MINUS, COMMA, LP, RP, AND, OR, NOT, SPACE, GT, LT, LEQ, GEQ, NEQ, KW_variables, SEMI, DOTDOT, KW_commands, KW_Print, KW_endvars, KW_terminate, KW_READFILE, KW_SUM, KW_PROD, EMPTY, KW_GETS, KW_IF, KW_THEN, KW_ELSE, KW_WHILE, KW_DO, KW_END, KW_TOSTRING, KW_MEAN, KW_CORR, KW_MEDIAN, KW_STDEV, KW_VAR, KW_RT, KW_POW, KW_GETF, KW_COV, KW_SUBS, KW_LINREG, VOID, KW_GETI, KW_TOFLOAT, KW_TOINT, CONCAT, EOF
           ]
    fun toString tok =
(case (tok)
 of (KW_let) => "let"
      ...
      in
        (case (lex(strm))
         of (Tok.SSTRING(_), _, strm') => expr_PROD_9(strm)
          ...
          | (Tok.KW_GETI, _, strm') =>
              tryProds(strm, [expr_PROD_2, expr_PROD_3, expr_PROD_7])
          | (Tok.KW_TOINT, _, strm') =>
              tryProds(strm, [expr_PROD_2, expr_PROD_3, expr_PROD_7])
          | (Tok.NUM(_), _, strm') =>
