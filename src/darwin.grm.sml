...
fun expr_PROD_4_ACT (exp_tupla, exp_tupla_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ts, tree, vars) = 
  ( exp_tupla)
fun expr_PROD_5_ACT (funcs_string, funcs_string_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ts, tree, vars) = 
  ( funcs_string)
fun expr_PROD_6_ACT (funcs_float, funcs_float_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ts, tree, vars) = 
  ( funcs_float)
fun expr_PROD_7_ACT (funcs_int, funcs_int_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ts, tree, vars) = 
  ( funcs_int)
fun expr_PROD_8_ACT (funcs_list, funcs_list_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ts, tree, vars) = 
  ( funcs_list)
...
fun expr_NT (strm) = let
      ...
      fun expr_PROD_5 (strm) = let
            ...
            in
              (UserCode.expr_PROD_5_ACT (funcs_string_RES, funcs_string_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ts_REFC, tree_REFC, vars_REFC),
                FULL_SPAN, strm')
            end
      fun expr_PROD_6 (strm) = let
            ...
            in
              (UserCode.expr_PROD_6_ACT (funcs_float_RES, funcs_float_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ts_REFC, tree_REFC, vars_REFC),
                FULL_SPAN, strm')
            end
      fun expr_PROD_7 (strm) = let
            ...
            in
              (UserCode.expr_PROD_7_ACT (funcs_int_RES, funcs_int_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ts_REFC, tree_REFC, vars_REFC),
                FULL_SPAN, strm')
            end
      fun expr_PROD_8 (strm) = let
            ...
            in
              (UserCode.expr_PROD_8_ACT (funcs_list_RES, funcs_list_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos), ts_REFC, tree_REFC, vars_REFC),
                FULL_SPAN, strm')
            end
      ...
      in
        (case (lex(strm))
         of (Tok.SSTRING(_), _, strm') => expr_PROD_9(strm)
          ...
          | (Tok.STUPLE(_), _, strm') => expr_PROD_9(strm)
          | (Tok.KW_GETI, _, strm') => expr_PROD_7(strm)
          | (Tok.KW_TOINT, _, strm') => expr_PROD_7(strm)
          | (Tok.KW_READFILE, _, strm') =>
              tryProds(strm, [expr_PROD_1, expr_PROD_5])
          | (Tok.KW_GETS, _, strm') =>
              tryProds(strm, [expr_PROD_1, expr_PROD_5])
          ...
          | (Tok.KW_SUM, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_PROD, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_MEAN, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_CORR, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_MEDIAN, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_STDEV, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_VAR, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_GETF, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_COV, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_TOFLOAT, _, strm') => expr_PROD_6(strm)
          | (Tok.KW_SUBS, _, strm') => expr_PROD_8(strm)
          | _ => fail()
        (* end case *))
      end
